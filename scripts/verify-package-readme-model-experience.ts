...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/browser-use/runtime': { kind: 'indirect', reason: 'Browser providers call the library to own resources and expose upstream MCP tools.' },
  ...
  'packages/client/ui-settings-automation': { kind: 'indirect', reason: 'Browser-side settings rows; the Playwright MCP provider owns every model-facing effect of the selection.' },
  ...
  'packages/security/wsl-pentest': { kind: 'indirect', reason: 'Six synchronous WSL tools; each model-visible output is the call\'s returned JSON result text, with no owned prompt, schema rendering, or persistent session state to structure.' },
  ...
  'packages/web/web-firecrawl': { kind: 'indirect', reason: 'The provider backend delegates model rendering to dsh-tool-web.' },
  'packages/client/ui-settings-web': { kind: 'none', reason: 'Browser-side UI plugin layer; registers nothing model-facing.' },
