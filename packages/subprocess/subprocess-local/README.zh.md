...
把提供方与它的消费方挂载在同一组合中，并完全按子进程服务的规定启动进程；本包只决定这些进程在宿主机上如何运行。在 Windows 上，非终端子进程与 `taskkill` 辅助进程会隐藏窗口，因此后台操作不会抢占焦点。遵循进程启动可见性设置的 GUI 窗口也会被隐藏。隐藏窗口只覆盖直接子进程：普通启动以 `inherit` 模式针对 runner 自身的隐藏控制台创建 target，因此 target 之后创建的 console 模式孙进程（如 `ping`）共享该控制台，而不是让系统为其分配一个新窗口。
