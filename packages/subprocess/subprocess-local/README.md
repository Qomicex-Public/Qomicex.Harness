...
Mount the provider beside its consumers and start processes exactly as the subprocess service specifies; this package decides only how those processes run on the host. On Windows, non-terminal children and `taskkill` helpers start with their windows hidden so background operations do not take focus. This also hides GUI windows that honor the process startup visibility setting. Hidden windows cover direct children only: an ordinary launch creates its target in `inherit` mode against the runner's own hidden console, so a console-mode grandchild the target later creates (a `ping`) shares that console instead of the system allocating a new window for it.
