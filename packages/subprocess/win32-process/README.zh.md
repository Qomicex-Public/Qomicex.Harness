...
- **ordinary Job runner 原语** — `CurrentTokenProcessSpawnOptions` 要求已解析的 `applicationName`、完整 target 环境，以及三个专用于 target stdin、stdout 与 stderr 的 runner CRT 描述符。`spawnCurrentTokenJobProcess()` 通过 Node 导出的 `uv_get_osfhandle()` 把这些描述符映射为 OS 句柄，拒绝无效结果，临时把句柄设为可继承，并通过 `STARTF_USESTDHANDLES` 传入。它使用 `CREATE_UNICODE_ENVIRONMENT` 传入排序后的 UTF-16LE 环境块，再以 suspended 状态通过 `CreateProcessW` 创建 target、把它分配给 unnamed kill-on-close Job，并只在分配后恢复。原始命令行 argv 项保持不变，runner 也可以关闭自己的 carrier 描述符，而不触碰 Node 自身的标准流。可选的 `console` 模式选择窗口投递方式：默认 `hidden` 添加 `CREATE_NO_WINDOW` 供叶子命令使用，`inherit` 则省略该标志，让 target 及其后代共享调用方的控制台。
- **隐藏控制台的建立** — `ensureHiddenConsole()` 在本进程没有控制台时为其分配一个，并立即用 `SW_HIDE` 隐藏其窗口。runner 在以 `inherit` 模式创建 target 前调用它：console 模式的孙进程（如 `ping`）若其父进程链没有任何控制台，系统会为其分配一个全新的控制台窗口，而 `CREATE_NO_WINDOW` 无法抑制它，因为创建标志不会传递给孙进程。`AllocConsole` 是幂等的——已存在控制台时报告拒绝访问，这正是期望结果——而空的 console 窗口仅表示没有可隐藏的对象。
