...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    const home = await tempRepo()
    try {
      await write(join(home, '.qomicex/AGENTS.md'), 'global default rule')
      ...
      expect(files.map(file => file.displayPath)).toEqual(['~/.qomicex/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
  ...
  it('expands a configured ~/.dsh home to the operating-system home directory', async () => {
    ...
    const home = await tempRepo()
    try {
      await write(join(home, '.qomicex/AGENTS.md'), 'global tilde rule')

      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@deepseek-ai/dsh-agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root, dshHome: '~/.qomicex' })

      expect(files).toEqual([{ absolutePath: join(home, '.qomicex/AGENTS.md'), displayPath: '~/.qomicex/AGENTS.md' }])
    } finally {
      vi.doUnmock('node:os')
      vi.resetModules()
      await rm(root, { recursive: true, force: true })
