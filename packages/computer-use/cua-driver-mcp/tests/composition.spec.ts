...
async function load(mode?: string): Promise<{ ctx: Context; root: string; model: ScreenshotModel }> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@deepseek-ai/dsh-computer-use-cua-driver-mcp', Provider],
  ])
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, JSON.stringify([...modules.keys()].map(name => ({
    id: name === '@deepseek-ai/dsh-computer-use-cua-driver-mcp' ? 'computer-use-driver' : undefined,
    name,
    config: name === '@deepseek-ai/dsh-computer-use-cua-driver-mcp'
      ? {
