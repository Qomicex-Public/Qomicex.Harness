...
- **Provider registration.** A provider contributes its catalog with `ctx.skills.registerProvider(caller, create)`; registration is synchronous, and the returned disposer removes the provider. `runtime` is a reserved provider name. The caller context selects the layer: an unscoped caller (a deployment row) registers globally, and a scoped caller (an agent preset's standing mount) registers into that scope's layer, where only agents joined to that preset see it.
