...
- **提供方注册。** 提供方用 `ctx.skills.registerProvider(caller, create)` 贡献目录；注册是同步的，返回的 disposer（资源释放）会移除该提供方。`runtime` 是保留的提供方名称。caller 上下文决定注册层：无作用域的调用方（部署行）注册进全局层，有作用域的调用方（agent preset 的常驻挂载）注册进该作用域层，只有加入该 preset 的 agent 能读到。
