...
选择**无**、**纯色**、**渐变**或**图片**。纯色与渐变使用 `#rrggbb` 颜色；渐变还接受方向角度。图片可来自本地上传或 `http(s)` URL。**遮罩强度**控制背景与内容之间的可读性遮罩，图片越花越需要调高。上传的图片以 Blob 存入浏览器 IndexedDB；设置文档只记录「正在使用该 Blob」，因此换一个浏览器在自行上传前不会显示图片。
