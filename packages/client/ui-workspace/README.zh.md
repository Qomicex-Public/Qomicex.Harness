...
- **Session 删除位于设置中，且需不可恢复确认**：会话可以归档但侧边栏绝不抹除；已归档会话的查看、恢复与永久删除由「已归档会话」设置页（[ui-settings-unarchive-sessions](../ui-settings-unarchive-sessions/README.zh.md)）负责，删除 Workspace 注册记录也不会删除 Session。
