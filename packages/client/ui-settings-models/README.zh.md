...
收起的「自定义设置」折叠区承载精选的额外字段：两个家族都有 `baseURL`（deepseek 的占位符显示公共端点）、各适配器自己的模型目录，以及适配器未提供的 pi-ai 路由的**显示名称**与 **API 协议**。Profile `headers` 仍是 `cordis.patch.yml` 或 Cordis 配置中的部署配置，Models 页面不提供编辑器。Provider ID 保持固定：它是 settings 的键、其他每个 namespace 与每一条已记录会话引用的名字，也是页面读不回、因而搬不走的凭据引用词干。推理等级是按模型的能力，因此按行编辑而不是按提供商编辑：提供商级的控件只可能被设成某些模型会拒绝的值。每个模型行可编辑 `id`、可选显示 `name`、可选 `contextWindow`/`maxTokens`、输入类型和推理强度；无关的模型字段在编辑后仍会保留。

`llm-deepseek` 的 DeepSeek 卡片编辑端点、凭据和模型目录。它使用 Messages，默认端点占位符为 `https://api.deepseek.com/anthropic`。

模型以磁贴形式呈现在设置面板上方。每个磁贴显示模型显示名（或 id）、上下文窗口与最大输出摘要及移除按钮；上方搜索框按 id 或显示名过滤。选中磁贴后内联展开该模型的设置：`id` 与显示 `name`、上下文窗口与最大输出的快捷档位分段轨（上方是精确数字输入框）、输入类型多选分段，以及 pi-ai 模型的推理强度编辑器。在添加行输入 id 并点击添加后，新行被自动选中，可立即编辑其设置。未声明输入类型的模型行优先显示已安装模型的输入类型，其次是提供商默认值，最后回退为文本。已知 pi-ai 提供商会加载已安装目录，不向端点发送请求；选中模型行不会写入覆盖值。显式输入选择具有优先权，包括为视觉模型设置的仅文本覆盖。分段编辑会保存所选类型，且至少保留一种。DeepSeek 写入 `inputModalities`，pi-ai 写入 `input`。DeepSeek 取消勾选图片时，还会移除 `imagePixelBudget` 与 `imageMaxBytes`，因为适配器在没有图片输入时拒绝这些限制。pi-ai 行额外展开**推理强度**：多选分段轨把模型提供的档位写为 `reasoningEfforts` 的键——新勾选的档位发送档位名作为线上拼写，已保留的档位沿用行内已存拼写；而当提供的档位没有超过 Off 时，写入 `reasoningEfforts: false`，即宿主架构的非推理声明。**默认思考等级**下拉把某个已提供档位写为 `defaultReasoningEffort`，作为新会话的预选档位；选择"继承"则不留该字段，沿用路由级默认；取消勾选已默认的档位会清空该字段，因为宿主会丢弃模型未提供的默认档位。容量按文本编辑，不可读时文本保留在屏幕上，保存时的拒绝能指名用户仍可修正的行；Off 以上已勾选却留空的档位会在任何写入前被拒绝，与适配器一致。在 `cordis.patch.yml` 中清除输入字段可恢复适配器继承；**恢复默认模型**会重置整个模型目录覆盖。仅声明上游模型实际能够处理的输入类型。
