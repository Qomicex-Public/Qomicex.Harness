...
模型以 chip 形式呈现在设置面板上方。每个 chip 显示模型显示名（或 id）、上下文窗口与最大输出摘要及移除按钮；上方搜索框按 id 或显示名过滤。选中 chip 后内联展开该模型的设置：`id` 与显示 `name`、上下文窗口与最大输出快捷 chip（旁边是精确数字输入框）、输入类型复选框，以及 pi-ai 模型的推理强度 chip 行。在添加行输入 id 并点击添加后，新行被自动选中，可立即编辑其设置。未声明输入类型的模型行优先显示已安装模型的输入类型，其次是提供商默认值，最后回退为文本。已知 pi-ai 提供商会加载已安装目录，不向端点发送请求；选中模型行不会写入覆盖值。显式输入选择具有优先权，包括为视觉模型设置的仅文本覆盖。修改复选框会保存所选类型，且至少保留一种。DeepSeek 写入 `inputModalities`，pi-ai 写入 `input`。DeepSeek 取消勾选图片时，还会移除 `imagePixelBudget` 与 `imageMaxBytes`，因为适配器在没有图片输入时拒绝这些限制。pi-ai 行额外展开**推理强度** chip 行：点击某档即选中该模型提供的档位并发送档位名，再次点击已选档位会清除该字段，改由已安装的模型目录决定；**禁用推理**为不能推理的模型写入 `reasoningEfforts: false`。容量按文本编辑，不可读时文本保留在屏幕上，保存时的拒绝能指名用户仍可修正的行；Off 以上已勾选却留空的档位会在任何写入前被拒绝，与适配器一致。在 `cordis.patch.yml` 中清除输入字段可恢复适配器继承；**恢复默认模型**会重置整个模型目录覆盖。仅声明上游模型实际能够处理的输入类型。
