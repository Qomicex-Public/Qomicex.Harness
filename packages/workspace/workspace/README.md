...
The API has two owners: `WorkspaceRegistry` creates, orders, and deletes projects, manages their Session accounting, and pins, unpins, archives, restores, or deletes Sessions; the `Workspace` entity exposes the display title, directory status, and Session projection. Pinning requires a known, unarchived Session; archiving clears its pin in the same durable write, and restoring does not restore that pin. `deleteSession` erases a Session's persistence artifacts first, then drops its account and archive-set entry, so an interrupted delete leaves a ghost session the next start filters out. Per-method contracts live in [src/index.ts](src/index.ts) and [src/entity.ts](src/entity.ts).
...
- **Removal never deletes data** — removing a project leaves its folder, files, and session histories in place; those sessions become ungrouped, and folder removal remains an absent capability, while session deletion is the separate `deleteSession` path ([decision](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.md)).
...
- **A live session cannot be deleted** — `deleteSession` refuses a session that is live in this process: its owning agent holds the write handle over the log being erased, so the agent must close first.
