...
- name: '@deepseek-ai/dsh-browser-use-stagehand-native'
  config:
    mode: launch
    headless: true
    model:
...
```sh
pnpm exec vitest run packages/browser-use/stagehand-native/tests
```
...
```sh
pnpm run build
env -u NODE_USE_ENV_PROXY DSH_STAGEHAND_E2E=1 pnpm exec vitest run --config vitest.e2e.config.ts packages/browser-use/stagehand-native/tests/native.e2e.ts
```
...
[SessionResources](../runtime/README.zh.md) 为每个确切的活动 Agent 管理延迟获取、串行执行和资源释放。Provider 保留浏览器使用注册，直到清理完成。[原生 Provider](src/index.ts) 通过现有 MCP 结果适配器注册工具，由该适配器把截图保存为持久附件。
...
[`stagehand_` 工具目录](../../../docs/tool-catalog.zh.md#deepseek-aidsh-browser-use-stagehand-native)定义导航、标签页管理、截图、操作、观察和提取。结果包含当前页面事实或验证后的结构化数据。支持的截图以持久图像附件呈现。错误保持可见，让模型在重试前检查状态。
