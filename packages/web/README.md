...
The `web/` packages let models search the public web and fetch HTTP(S) pages through the `web_search` and `web_fetch` tools. Deployments can choose Firecrawl, Exa, Perplexity, or DeepSeek for search and Firecrawl or anonymous HTTP(S) access for fetch; availability and resource limits depend on the configured provider. The shipped base composition selects Firecrawl for both capabilities, and the General settings page switches either backend. Use this family for search and page retrieval, not interactive browsing, content extraction, or per-URL policy enforcement. Models receive consistent tool behavior, cancellation, and error reporting when providers change.
...
| [`web-firecrawl/`](web-firecrawl/README.md) | Searches the web and scrapes pages through Firecrawl | registers on `ctx.web` |
