...
describe('firecrawl egress', () => {
  it('goes through the proxy', async () => {
    const p = new FirecrawlSearchProvider({ apiKey: 'probe', baseURL: 'http://firecrawl-probe.invalid' })
    expect(await observe(() => p.search({ query: 'probe' }))).toEqual(['REQ http://firecrawl-probe.invalid/v2/search'])
  })
})
