...
describe('FirecrawlSearchProvider redirect policy', () => {
  it.each([301, 302, 303, 307, 308])('rejects HTTP %i before contacting Location', async (status) => {
    ...
    const provider = new FirecrawlSearchProvider({ apiKey: TEST_API_KEY, baseURL: `${redirectOrigin}/${status}` })
...
describe('FirecrawlFetchProvider redirect policy', () => {
  it('rejects a cross-origin redirect before contacting Location', async () => {
    targetRequests.length = 0
    const provider = new FirecrawlFetchProvider({ apiKey: TEST_API_KEY, baseURL: `${redirectOrigin}/307` })

    await expect(provider.fetch({ url: 'https://page.test' }))
      .rejects.toMatchObject({ code: 'WEB_PROVIDER_ERROR' })
    expect(targetRequests).toHaveLength(0)
  })
})

describe('default fetch behavior the policy rejects', () => {
  it('shows default 307 following forwards the custom credential and POST body', async () => {
    targetRequests.length = 0
    const body = JSON.stringify({ query: 'private redirect query', sources: ['web'] })
