...
- **被授权目录必须由调用者拥有；仅有所有权的目录会自授 `WRITE_OWNER`。** 所有者隐式获得的只有 `READ_CONTROL` 与 `WRITE_DAC`；标签位于 SACL，因此合并应用还需要 `WRITE_OWNER`。DACL 只授予 Modify 的目录会让首次（合并）应用返回拒绝访问，`AclWriteGrant.add` 随即通过一次只改 DACL 的自授——给目录所有者补上可继承的 `WRITE_OWNER`——并重试一次。完全控制目录（即正常工作区情形）无需这两步；自授的 ACE 会留给后续会话复用。
