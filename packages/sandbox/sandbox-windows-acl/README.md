...
- **Granted directories must be caller-owned; an owner-only directory self-grants `WRITE_OWNER`** — the owner's implicit rights cover only `READ_CONTROL` and `WRITE_DAC`; the label lives in the SACL, so the combined apply additionally needs `WRITE_OWNER`. A directory whose DACL grants only Modify therefore fails the first (combined) apply with access denied, and `AclWriteGrant.add` answers it with one DACL-only self-grant of `WRITE_OWNER` to the directory's owner and one retry. A Full-control directory (the normal workspace case) needs neither step; the self-granted ACE stands for later sessions.
...
| File | Role |
|---|---|
| [`src/index.ts`](src/index.ts) | `AclSandbox`: restricted-token policy, DACL and label grants, fail-closed spawn and dispose |
...
| [`src/token.ts`](src/token.ts) + [`src/acl.ts`](src/acl.ts) | Win32 token, DACL, and mandatory-label primitives behind the sandbox |
