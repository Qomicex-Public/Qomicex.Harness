...
`delete(id)` 移除整个会话自有目录——所有 generation、写锁文件以及任何会话本地产物——因此抹除是彻底的。后端先获取该会话目录的跨进程写锁：本进程或其他进程中活跃的写所有者会以 `SessionAlreadyOwnedError` 拒绝抹除，而不是向已删除的目录写入；锁只在移除前后获取与释放。该 id 的 cold-log memo 条目随之丢弃。未知 id 拒绝为 `SessionPersistenceNotFoundError`。
