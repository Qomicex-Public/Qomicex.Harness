...
`delete(id)` removes the whole session-owned directory — every generation, the write lock file, and any session-local artifact — so the erase is complete. The backend claims the session directory's cross-process write lock first: a live write owner in this or another process refuses the erase with `SessionAlreadyOwnedError` instead of writing into a removed directory, and the lock is claimed and released around the removal only. The cold-log memo entry for the id is dropped with it. An unknown id rejects `SessionPersistenceNotFoundError`.
