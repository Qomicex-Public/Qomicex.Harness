...
服务级 `flush()` 排空每个活跃写句柄已路由的事件并把其会话实体化，效果与各句柄自己的 `flush` 完全相同；失败按会话聚合为一个 `AggregateError` 而不中途放弃清扫，清扫途中被关闭的句柄视同已 flush，因为 close 本身会持久排空。`delete(id)` 是 `create` 的销毁对应操作：它抹除该会话及其全部持久化产物，使其从 `stat`/`list` 消失并让该 id 可重新 `create`。未知 id 拒绝为 `SessionPersistenceNotFoundError`；本进程或其他进程的写所有者仍持有时拒绝为 `SessionAlreadyOwnedError`。从未实体化的会话没有可抹除的产物。
