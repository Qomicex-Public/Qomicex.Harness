...
Native packages that load a library by an explicit path ship whole in the unpacked plane: electron-builder unpacks every native binary, and a package whose load path derives from module resolution — the Cua Driver platform package opens its DLL from the directory of its resolved `package.json` — additionally unpacks the whole package so that directory is real. The Host rewrites each resolved module URL onto its unpacked file when that file exists, so a native library receives a real filesystem path while every packed module keeps reading from the archive.
