...
应用菜单第一项“**关于 Qomicex Harness**”打开 Electron 原生关于面板，展示应用图标、产品名称和当前安装的发布版本。菜单文案跟随桌面壳的语言。macOS 的隐藏、隐藏其他、显示全部和退出条目使用本地化文案，隐藏和退出条目包含 Qomicex Harness 产品名称。这些条目保留原生动作和快捷键。macOS 从应用包读取图标，因此未打包的开发启动会显示 Electron 图标；Windows 使用随包分发的 PNG。
...
Qomicex 原稿位于 `resources/icon.png` 和 `resources/icon.svg`；`resources/icon-windows.png` 与 `resources/icon-macos.png` 是同一原稿的透明 1024×1024 导出。electron-builder 为 Windows 应用、安装程序和卸载程序生成多尺寸 ICO（[Windows 图标要求](https://learn.microsoft.com/en-us/windows/apps/design/iconography/app-icon-construction)）。安装页面在两种主题下使用匹配的图案；卸载程序的欢迎和完成页共用 `installer/assets/uninstaller-sidebar.png`，准备阶段将其转换为 164×314 BMP。

macOS PNG 是同一原稿的透明 1024×1024 导出，ICNS 打包时由系统应用圆角遮罩。它是扁平图标，并非 Icon Composer 文档。Apple 的[应用图标指南](https://developer.apple.com/design/human-interface-guidelines/app-icons)要求向 Icon Composer 提供未遮罩的图层；这些输入需要在 macOS 上单独导出，不能复用已做圆角的 ICNS 图案。发布前须在支持的 macOS 版本中验收 Finder 和 Dock 的显示效果。
...
Electron 壳在构建期将自己的一方 workspace 依赖打入 `lib/main.js`。打包在仓库构建之后，对该包单独跑一次 tsdown（配置中对 `@deepseek-ai/*` 的 `deps.alwaysBundle`）：`build:official` 内部的 workspace 级 tsdown 会以内联设置构建该包并丢弃 `alwaysBundle`，其产出的主进程会保留打包 `node_modules` 无法解析的一方 import。bundle 同时带入 `js-yaml` 等三方闭包，主进程运行期不从打包的 `node_modules` 解析这些包。否则打包闭包会被截断：vendored 清单声明 `workspace:~` 范围，electron-builder 收集器无法解析并静默跳过（cordis 的 `@deepseek-ai/cosmokit`）。三方与原生依赖保持外联由收集器打包；renderer 继续走独立的 Vite bundle。
...
首个致命弹窗打开前，Electron 会向平台日志目录（`app.getPath('logs')`：macOS 为 `~/Library/Logs/Qomicex Harness`，Windows 与 Linux 为应用 `userData` 目录下的 `logs`）写入一份崩溃报告，最多等待写入一秒；写入缓慢或失败时弹窗不带路径。文件 `crash-<UTC 时间>-<source>.log` 记录来源（`host` 为 Host 退出、`web-boot` 为渲染进程启动失败、`renderer` 为渲染进程或文档失败、`main` 为壳自身错误）、后端是否已就绪、应用与运行时版本、包含可枚举属性与 cause 链的错误（截至 256 KiB）、Host 在退出前通过 IPC 报告启动失败时自己的 inspect 错误（最多 64 KiB），以及主窗口最近的 error 级 console 输出（最多 64 KiB）。因此 Host 退出报告包含保留的 stderr 尾部，其中可能含有插件输出。关闭过程中的致命失败只写报告、不弹窗。平台支持时文件仅所有者可读；启动时保留最新十份报告并删除更早的，不触碰目录中的其他文件。
...
解压失败时，安装器会把 7-Zip 的结果和完整错误输出写入更新缓存目录 `%LOCALAPPDATA%\<按包名派生>-updater\installer-logs\extract-failure-<时间戳>.log`（当前为 `@deepseek-aidsh-desktop-updater`），并在弹窗中显示首条错误行和 **复制错误信息** 按钮；静默安装只写入报告。未签名的 Windows 构建（`DSH_DESKTOP_UNSIGNED=1`）会将安装包命名为 `qomicex-harness-<版本>-win-x64-unsigned.exe`，以免被误当作发布产物。
...
Windows 安装程序使用原生 NSIS 页面，提供亮暗配色、系统阴影、可编辑的安装目录，以及默认勾选立即启动的完成页。安装仅面向当前用户。点击安装或按 Enter 均校验当前路径；新安装位置必须为空，非空位置必须是已登记的安装目录。受影响安装路径中的运行程序不再阻止安装：安装程序会结束该安装自身的进程，使安装无需人工值守即可继续。静默更新先等待受影响应用退出至多十秒，再将其结束；两条路径仅在进程无法被终止时，才回退到运行中应用提示并以退出码 2 结束。其他目录中的同名应用不受影响，不阻止安装。
