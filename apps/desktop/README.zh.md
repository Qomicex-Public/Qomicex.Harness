...
Electron 壳在构建期将自己的一方 workspace 依赖打入 `lib/main.js`（tsdown 对 `@deepseek-ai/*` 使用 `noExternal`），主进程运行期不从打包的 `node_modules` 解析这些包。否则打包闭包会被截断：vendored 清单声明 `workspace:~` 范围，electron-builder 收集器无法解析并静默跳过（cordis 的 `@deepseek-ai/cosmokit`）。三方与原生依赖保持外联由收集器打包；renderer 继续走独立的 Vite bundle。
