...
const ARTIFACT = /(?:^|\/)qomicex-harness-(?<version>.+)-(?:mac|win)-(?:arm64|x64)(?:-unsigned)?\.(?:exe|dmg|zip)$/u
      ...
      const page = await Promise.race([
        new Promise<{ keys: string[]; next: string | undefined }>((resolveListing, rejectListing) => {
          cos.getBucket({
            ...
            Prefix: `${update.binaryKeyPrefix}/qomicex-harness-`, ...marker === undefined ? {} : { Marker: marker },
          }, (error, data) => {
