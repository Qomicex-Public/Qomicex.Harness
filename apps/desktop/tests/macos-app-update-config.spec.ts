...
async function fixture(): Promise<{ appPath: string; resourcesDir: string }> {
  ...
  roots.push(root)
  const appPath = join(root, 'Qomicex Harness.app')
...
describe('macOS packaged updater configuration', () => {
  ...
  it('writes and verifies the fixed release feed before signing', async () => {
    ...
    expect(createMacOSAppUpdateConfig(update, 'qomicex-harness-updater')).toEqual({
      provider: 'generic',
      url: update.publicUrl,
      channel: 'nightly',
      updaterCacheDirName: 'qomicex-harness-updater',
    })
    await writeMacOSAppUpdateConfig(paths.resourcesDir, update, 'qomicex-harness-updater')
    await expect(verifyMacOSAppUpdateConfig(paths.appPath, update, 'qomicex-harness-updater')).resolves.toBeUndefined()
  })
  ...
  it('rejects another updater cache directory when the signed value is known', async () => {
    ...
    await expect(verifyMacOSAppUpdateConfig(paths.appPath, update, 'qomicex-harness-updater'))
      .rejects.toThrow(/expected qomicex-harness-updater/u)
  })
})
