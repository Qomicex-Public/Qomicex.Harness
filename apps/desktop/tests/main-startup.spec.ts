...
const harness = await vi.hoisted(async () => {
  ...
  class FakeWindow extends EventEmitter {
    ...
    async loadURL(url: string) {
      ...
      if (url === 'qomicex-app://app/') navigated.resolve()
    }
...
function invoke(channel: string, origin = channel === DESKTOP_IPC.boot ? 'app' : 'shell', ...args: unknown[]): unknown {
  ...
  return handler({ senderFrame: { url: `qomicex-app://${origin}/index.html` } }, ...args)
}
...
describe('desktop main startup', () => {
  it('routes shell update documents and assets through the registered main protocol handler', async () => {
    ...
    try {
      ...
      for (const [name, mime] of [
        ...
        const response = await handler(new Request(`qomicex-app://shell/${name}`))
      ...
      const head = await handler(new Request('qomicex-app://shell/update-dialog.html', { method: 'HEAD' }))
      expect(head.status).toBe(200)
      expect(await head.text()).toBe('')
      expect((await handler(new Request('qomicex-app://shell/update-dialog.html', { method: 'POST' }))).status).toBe(405)
      expect((await handler(new Request('qomicex-app://shell/%'))).status).toBe(400)
      expect((await handler(new Request('qomicex-app://shell/%2e%2e%2fpackage.json'))).status).toBe(403)
      expect((await handler(new Request('qomicex-app://shell/missing.html'))).status).toBe(404)
      expect((await handler(new Request('qomicex-app://other/update-dialog.html'))).status).toBe(404)
    } finally {
  ...
  it('serves shell dialogs and their assets without forwarding them to the Host', async () => {
    ...
    for (const file of ['update-dialog.html', 'update-dialog.js', 'update-dialog.css', 'update-close.svg', 'mandatory-update.html']) {
      const request = new Request(`qomicex-app://shell/${file}`)
    ...
    expect((await handler(new Request('qomicex-app://unknown/update-dialog.html'))).status).toBe(404)
  })
  ...
  it.each([
    ...
    expect(harness.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
      type: 'info', title: zh ? '关于 Qomicex Harness' : 'About Qomicex Harness', message: 'Qomicex Harness',
  ...
  it.each(['darwin', 'win32', 'linux'] as const)('limits native titlebar styling to macOS on %s', async (platform) => {
    ...
    expect(window.urls).toEqual(['qomicex-app://app/'])
    if (platform === 'darwin') {
  ...
  it('serves packaged shell documents instead of rejecting the shell origin', async () => {
    ...
    await harness.preparing.promise
    const handler = harness.protocolHandle.mock.calls[0]![1]
    const request = new Request('qomicex-app://shell/update-dialog.html')
    ...
    expect((await handler(new Request('qomicex-app://foreign/index.html'))).status).toBe(404)
  })
  ...
  it('follows the Windows primary document language and palette without trusting other frames', async () => {
    ...
    listener({ ...event, senderFrame: { url: 'qomicex-app://app/' } }, 'zh-CN', '#ffffff', '#000000')
    ...
    window.setTitleBarOverlay.mockClear()
    window.webContents.mainFrame.url = 'qomicex-app://unowned/index.html'
  ...
  it('maps Windows caption menus to localized native commands and rejects foreign popup requests', async () => {
    ...
    expect(harness.menu.buildFromTemplate.mock.lastCall![0].map(item => item.label ?? item.type)).toEqual([
      '关于 Qomicex Harness', 'separator', '检查更新…', 'separator', '退出',
    ])
  ...
  it('attaches Host socket credentials only to the owned application origin and window', async () => {
    ...
    const details = { url: 'ws://127.0.0.1:3080/api/remote.mux', webContentsId: 42, requestHeaders: { Origin: 'qomicex-app://app' } }
  ...
  it('holds boot injections until the Host is ready and rejects foreign boot callers', async () => {
    ...
    let settled = false
    const boot = Promise.resolve(handler({ senderFrame: { url: 'qomicex-app://app/' } })).then((value) => { settled = true; return value })
    await Promise.resolve()
    expect(settled).toBe(false)
    expect(harness.windows[0]!.urls).toEqual(['qomicex-app://app/'])
    harness.prepared.resolve()
    await harness.hostStarted.promise
    expect(settled).toBe(false)
    harness.hosts[0]!.ready.resolve()
  ...
  it('opens message links externally while retaining same-origin application navigation', async () => {
    ...
    window.webContents.emit('will-navigate', internal, 'qomicex-app://app/session/task-1')
  ...
  it('blocks subsequent product operations without stopping the Host and clears only on a fresh no-force policy', async () => {
    ...
    const unowned = [
      { ...owned, sender: {} },
      { ...owned, senderFrame: { url: 'qomicex-app://app/index.html' } },
  ...
  it.each([false, true])('restores a cleanly stopped Host after installer failure and retains mandatory blocking: %s', async (mandatory) => {
    ...
    await vi.waitFor(() => { expect(harness.windows[0]!.urls).toEqual(['qomicex-app://app/', 'qomicex-app://app/']) })
  ...
  it('accepts Web fatal reports only from the primary application frame', async () => {
    ...
    await harness.preparing.promise
    const window = harness.windows[0]!
    const frame = { url: 'qomicex-app://app/' }
  ...
  it('ignores subresource failures and navigation cancellation but reports a failed main document', async () => {
    ...
    await harness.preparing.promise
    const window = harness.windows[0]!
    window.webContents.emit('did-fail-load', {}, -2, 'failed', 'qomicex-app://app/image.png', false)
    window.webContents.emit('did-fail-load', {}, -3, 'aborted', 'qomicex-app://app/', true)
    expect(harness.dialog.showMessageBox).not.toHaveBeenCalled()
    window.webContents.emit('did-fail-load', {}, -2, 'failed', 'qomicex-app://app/', true)
    await harness.dialogShown.promise
  ...
  it('attaches the primary window\'s error-level console output to a running-phase Host crash report', async () => {
    ...
    await Promise.resolve(invoke(DESKTOP_IPC.boot))
    const window = harness.windows[0]!
    window.webContents.emit('console-message', { level: 'info', message: 'boot ok', sourceId: 'qomicex-app://app/assets/entry.js', lineNumber: 1 })
    window.webContents.emit('console-message', {
      ...
      sourceId: 'qomicex-app://app/assets/entry.js', lineNumber: 12,
    })
    ...
    expect(report.rendererConsole).toEqual(['qomicex-app://app/assets/entry.js:12 client-modules: bundle script plugins/??a/client.js&rev=1 failed to load'])
