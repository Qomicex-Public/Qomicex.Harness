...
function setup() {
  dom = new JSDOM('<html><body><button>Product action</button><div data-windows-menu></div></body></html>', { url: 'qomicex-app://app/' })
  ...
  expect(post).toHaveBeenCalledWith({ type: 'dsh-mandatory-connect' }, 'qomicex-app://shell', [channel.port2])
...
it('uses an in-page frame below the caption and reuses it on updates', () => {
  ...
  expect(f.frame.src).toBe('qomicex-app://shell/mandatory-update.html')
...
it('rejects forged window events and accepts actions only through the transferred private port', async () => {
  ...
  dom.window.dispatchEvent(new dom.window.MessageEvent('message', { source: f.frame.contentWindow, origin: 'qomicex-app://shell', data }))
