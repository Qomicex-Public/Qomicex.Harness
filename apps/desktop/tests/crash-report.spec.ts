...
const input = (overrides: Partial<CrashReportInput> = {}): CrashReportInput => ({
  ...
  rendererConsole: [],
  app: { name: 'Qomicex Harness', version: '0.1.7', platform: 'win32', arch: 'x64', electron: '44.0.0', node: '24.18.1', locale: 'zh_CN' },
...
it('renders the facts header, the inspected error with its properties and cause, and the renderer console', () => {
  const text = renderCrashReport(input({
    ...
    rendererConsole: ['qomicex-app://app/assets/entry.js:12 client-modules: bundle script plugins/??a/client.js&rev=1 failed to load'],
  }))
  expect(text).toContain('time: 2026-09-22T10:30:00.123Z\nsource: host\nphase: running\napp: Qomicex Harness 0.1.7\nplatform: win32 x64\nelectron: 44.0.0\nnode: 24.18.1\nlocale: zh_CN\n')
  ...
  expect(text).toContain('--- renderer console (error level, oldest first) ---\nqomicex-app://app/assets/entry.js:12 client-modules: bundle script')
