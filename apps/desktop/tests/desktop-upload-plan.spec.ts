...
async function fixture(
  ...
  const base = `qomicex-harness-${version}-${os}-${arch}`
...
describe('desktop upload plan', () => {
  it('publishes fixed feeds referencing versioned binaries without overriding CDN cache policy', async () => {
    ...
    expect(plan.artifacts.map(artifact => artifact.key)).toEqual([
      'dsh-desk/bin/win-x64/qomicex-harness-1.2.3-win-x64.exe',
      'dsh-desk/bin/win-x64/qomicex-harness-1.2.3-win-x64.exe.blockmap',
    ...
    expect(load(plan.artifacts[2]!.contents!)).toMatchObject({
      version: '1.2.3',
      files: [{
        url: 'https://download.deepseek.com/dsh-desk/bin/win-x64/qomicex-harness-1.2.3-win-x64.exe',
  ...
  it('validates macOS artifacts and puts channel metadata last', async () => {
    ...
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'qomicex-harness-1.2.3-mac-arm64.dmg',
      'qomicex-harness-1.2.3-mac-arm64.zip',
      'qomicex-harness-1.2.3-mac-arm64.zip.blockmap',
      'nightly-mac.yml',
      'latest-mac.yml',
    ])
  ...
  it('uploads the prerelease channel metadata emitted by electron-builder', async () => {
    ...
    const plan = await createDesktopUploadPlan('mac-arm64', paths)
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'qomicex-harness-1.2.3-alpha.4-mac-arm64.dmg',
      'qomicex-harness-1.2.3-alpha.4-mac-arm64.zip',
      'qomicex-harness-1.2.3-alpha.4-mac-arm64.zip.blockmap',
      'nightly-mac.yml',
    ])
  })
  ...
  it('validates the Windows installer with the emitted external blockmap and production destination', async () => {
    ...
    const plan = await createDesktopUploadPlan('win-x64', paths)
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'qomicex-harness-2.0.0-win-x64.exe',
      'qomicex-harness-2.0.0-win-x64.exe.blockmap',
      'nightly.yml',
      'latest.yml',
    ])
    expect(plan).toMatchObject({
  ...
  it.each(['missing', 'empty'])('rejects a %s Windows blockmap before publishing its feed', async (condition) => {
    ...
    const path = join(paths.artifactsRoot, 'qomicex-harness-1.2.3-win-x64.exe.blockmap')
  ...
  it('rejects stale architecture metadata and modified updater bytes', async () => {
    ...
    const zipPath = join(paths.artifactsRoot, 'qomicex-harness-1.2.3-mac-arm64.zip')
    ...
    await writeFile(metadataPath, `${JSON.stringify({
      version: '1.2.3',
      files: [{
        url: 'qomicex-harness-1.2.3-mac-x64.zip',
