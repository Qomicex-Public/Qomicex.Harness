...
it.each(['qomicex-app://shell/plugin-manager.html', 'qomicex-app://other/index.html', 'https://shell/startup.html', 'http://example.com/'])('exposes only the carrier marker to %s', async (url) => {
...
it('exposes asynchronous boot only to the local application document', async () => {
  vi.stubGlobal('location', new URL('qomicex-app://app/'))
  await import('../src/preload-app.ts')
...
it('exposes a directory picker only to the local application document', async () => {
  ...
  for (const url of ['qomicex-app://shell/startup.html', 'https://example.com/']) {
    vi.resetModules()
    electron.contextBridge.exposeInMainWorld.mockClear()
    vi.stubGlobal('location', new URL(url))
    await import('../src/preload-app.ts')
...
it.each(['qomicex-app://app/', 'qomicex-app://shell/plugin-manager.html', 'https://example.com/'])(
  'installs Windows appearance only for the application document (%s)', async (url) => {
    vi.stubGlobal('location', new URL(url))
    await import('../src/preload-app.ts')
    expect(syncWindowsAppearance).toHaveBeenCalledTimes(url === 'qomicex-app://app/' ? 1 : 0)
  },
)
...
it.each(['win32', 'darwin'] as const)('installs the embedded mandatory UI only in the Windows app document (%s)', async (platform) => {
  ...
  for (const url of ['qomicex-app://app/', 'qomicex-app://shell/mandatory-update.html', 'https://example.com/']) {
    vi.resetModules()
    vi.mocked(installMandatoryUpdateOverlay).mockClear()
    vi.stubGlobal('location', new URL(url))
    await import('../src/preload-app.ts')
    expect(installMandatoryUpdateOverlay).toHaveBeenCalledTimes(platform === 'win32' && url === 'qomicex-app://app/' ? 1 : 0)
  }
})
