...
async function windowAt(url) {
  if (process.platform === 'win32' && url === 'qomicex-app://shell/mandatory-update.html') {
    return mandatoryFrameDriver(await windowAt('qomicex-app://app/'))
  }
...
async function dialogWith(message) {
  ...
  await waitFor(async () => {
    for (const window of BrowserWindow.getAllWindows()) {
      if (window.webContents.getURL() !== 'qomicex-app://shell/update-dialog.html') continue
      try {
...
async function qualify() {
  ...
  try {
    ...
    mainWindow = await windowAt('qomicex-app://app/')
    ...
    assert.equal(BrowserWindow.getAllWindows().some(window => window.webContents.getURL() === 'qomicex-app://shell/update-dialog.html'), false)
    ...
    const mandatory = await windowAt('qomicex-app://shell/mandatory-update.html')
    ...
    server.policy('force')
    checkMenu.click()
    let forcedRecovery = await windowAt('qomicex-app://shell/mandatory-update.html')
