...
protocol.registerSchemesAsPrivileged([{ scheme: 'qomicex-app', privileges: { standard: true, secure: true, supportFetchAPI: true } }])
const cases = []
const presentations = []
...
async function main() {
  ...
  try {
    ...
    await scenario('mandatory-real-window-policy-download-retry-and-clear', async () => {
      protocol.handle('qomicex-app', async request => {
      ...
      await parent.loadURL('qomicex-app://app/')
      const f = await fixture('hold-download')
