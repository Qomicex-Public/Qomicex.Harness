...
vi.mock('electron', () => ({
  ...
  BrowserWindow: class {
    ...
    webContents = { mainFrame: { url: 'qomicex-app://app/' }, setWindowOpenHandler: vi.fn(), on: vi.fn(), once: vi.fn(), send: vi.fn(), openDevTools: vi.fn() }
...
it('starts the Host for welcome onboarding and opens the workspace on skip without quitting', async () => {
  ...
  expect(state.loadWorkspace).toHaveBeenCalledExactlyOnceWith('qomicex-app://app/')
  expect(state.showWorkspace).not.toHaveBeenCalled()
