...
async function main(): Promise<void> {
  ...
  session.defaultSession.webRequest.onBeforeSendHeaders({ urls: ['ws://127.0.0.1/*'] }, (details, callback) => {
    ...
    if (headers.origin !== 'qomicex-app://app') { callback({ cancel: true }); return }
  ...
  const assertMainApplication = (event: IpcMainInvokeEvent): BrowserWindow => {
    ...
    if (owner === undefined || event.sender !== owner.webContents || event.senderFrame !== owner.webContents.mainFrame
      || !event.senderFrame.url.startsWith('qomicex-app://app/')) throw new Error('Rejected Platform command')
    return owner
  }
  ...
  app.setAboutPanelOptions({
    applicationName: 'Qomicex Harness',
  ...
  if (app.isPackaged || process.env.DSH_DESKTOP_DEV_APP === '1') app.setAsDefaultProtocolClient('qomicex')
