...
| # | 问题 | 决策 | 影响与核实结论 |
|---|---|---|---|
| Q13 | 品牌资源 | **已处理（源文件归档到 `brand/source/`）** | 原始两个 SVG **都不是矢量图** —— 是 SVG 外壳 + 内嵌 base64 PNG 位图（共 11 个图层）。已提取全部图层并生成完整资产：应用图标（`.ico` 7 尺寸 / `.icns` 8 尺寸 / `.png` 9 尺寸）、应用内 mark（9 个尺寸）、横向 logo。**选定图层为 `mark-layer1-501x625.png` 与 `lockup-layer2-2161x520.png`**（其余共享图层是残缺导出，填充率仅 41.7% / 14.1%）。原始文件归档在 `brand/source/`，根目录不再保留副本。**建议向设计方索取真正的矢量源文件。** |
...
**选定的图层**：图标用 `mark-layer1-501x625.png`（填充率 71.5%，完整），logo 用 `lockup-layer2-2161x520.png`（填充率 30.0%）。其余共享图层是同一图形的残缺导出（41.7% / 14.1%），已弃用。
...
整体落在薄荷绿/青绿区间 **`#04CD95 – #16E0AD`**，建议主色 **`#0AD8A8`**（深色主题提亮至 `#12E0B0`）。
