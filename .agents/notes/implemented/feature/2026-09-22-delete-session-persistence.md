...
The client Session list is pull-based, so a host-side erasure must announce itself: after the durable accounts are consistent, `WorkspaceRegistry.deleteSession` emits `workspace/session-erased`, and the Session Controller relays it as the existing `api-session/removed` edge that every connected client already applies to drop its row and flag an open instance. Without the relay the row survives until a reconnect and opening it fails with `session/not-found`.

## Verification

The shared persistence contract suite (`runPersistenceContract`) carries the seam-wide cases: erase hides a stored Session from `stat`/`list`/`open` and frees its id for a new create, and an absent id or a live write owner refuses without erasing anything. Registry coverage lives in `workspace.spec.ts` (accounting drop, archive-set removal, refusals, persistence-failure propagation, the erasure announcement); controller coverage in `workspace-controller.host.spec.ts` (stable failure mapping) and `model.client.spec.ts` (archive-set drop on success, retention on failure), with the Session-list relay pinned in `controller.host.spec.ts`; UI coverage in `components.client.spec.tsx` (the dialog stays disabled until the acknowledgement is checked, cancel erases nothing, a rejection stays a console diagnostic) and `browser-plugin.client.spec.tsx` (the injected write). `pnpm run build` compiles both faces.
