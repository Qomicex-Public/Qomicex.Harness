...
共享 Web 插件管理器负责包变更和激活；Electron 保留 profile 准备和原生恢复。[Web 薄壳决策](2026-09-10-desktop-web-wrapper.zh.md)负责这些职责。打包期运行时项目携带自己审定的 `allowBuilds`——`node-pty`、`koffi`、`fs-ext`、打包的 `dsh-subprocess-local`，以及本地判断模型平台二进制 postinstall 所需的 `node-llama-cpp`——因为核心闭包成员的构建脚本未获批准时，`strictDepBuilds` 下它是硬性安装失败。
